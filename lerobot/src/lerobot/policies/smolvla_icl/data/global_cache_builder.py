...
def build_global_demo_cache(
    *,
    ...
    if sidecar.stats_fingerprint != train_stats.fingerprint:
        raise ValueError("pairing sidecar 与 train stats artifact 不一致。")
    ...
    if state_key not in dataset.meta.features or state_key not in train_dataset_stats:
        raise KeyError(f"Dataset feature 或 Train stats 中缺少 State key: {state_key!r}。")
    ...
    for demo_id, episode_index in sorted(sidecar.demo_id_to_episode.items()):
        ...
        if cache_path.is_file() and not overwrite:
            ...
            if cached.cache_identity != identity:
                raise ValueError(
                    f"已有 Global cache 身份不匹配：{cache_path}；"
                    "请使用 --overwrite 重新生成。"
                )
